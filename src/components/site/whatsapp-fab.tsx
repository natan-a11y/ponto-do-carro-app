...
const fullMessage = "Olá, tudo bem? Seja bem-vindo à Carro Venda Rápida. Quer vender seu veículo em até 24h? Entre em contato conosco!";
...
export default function WhatsAppFab() {
    ...
    const whatsAppMessage = "Olá! Tenho interesse em vender meu veículo em até 24h.";

    return (
        <div className="fixed bottom-6 right-6 z-50">
            {isOpen && (
                <div className="bg-card p-4 rounded-lg shadow-2xl border w-72 mb-4 animate-in fade-in-0 slide-in-from-bottom-5">
                    <button onClick={() => setIsOpen(false)} className="absolute top-2 right-2 text-muted-foreground hover:text-foreground">
                        <X size={18} />
                    </button>
                    <p className="text-sm text-foreground mb-4 pr-4">{displayedMessage}<span className="inline-block w-1 h-4 bg-foreground animate-ping ml-1"></span></p>
                    <Button className="w-full bg-green-500 hover:bg-green-600 text-white" asChild>
                         <a href={getWhatsAppLink(undefined, whatsAppMessage)} target="_blank" rel="noopener noreferrer">
                            Entre em contato
                        </a>
                    </Button>
                </div>
            ...
            <Button
                ...
                className={cn(
                    "rounded-full w-16 h-16 bg-green-500 hover:bg-green-600 shadow-lg text-white flex items-center justify-center",
